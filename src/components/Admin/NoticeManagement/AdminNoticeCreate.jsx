// src/components/NoticeManagement/AdminNoticeCreate.jsx
...
const AdminNoticeCreate = () => {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    console.log('공지사항 등록:', { title, content, date: currentDateTime });
    alert('공지사항이 등록되었습니다.');
    setTitle('');
    setContent('');
      ...
      <div className="admin-notice-create-content">
        ...
        <form onSubmit={handleSubmit} className="admin-notice-create-form">
          ...
          <div className="admin-notice-create-form-buttons">
            <button type="submit" className="admin-notice-create-register-button">등록</button>
            <button type="button" onClick={handleCancel} className="admin-notice-create-cancel-button">취소</button>
          </div>
        </form>
      </div>
    </div>
