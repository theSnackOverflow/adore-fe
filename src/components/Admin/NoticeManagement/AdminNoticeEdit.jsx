...
import axiosInstance from '../../../lib/axiosInstance'; // Axios 설정 가져오기
...
const AdminNoticeEdit = () => {
  ...
  const handleSearch = () => {
    ...
    fetchNotice(noticeId); // 공지사항 번호로 데이터 조회
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axiosInstance.patch(
        `/api/admin/notification/update?id=${noticeId}&state=${state}`, // id와 state를 쿼리 파라미터로 전달
        {
          title,
          content,
        }
        ...
        <form onSubmit={handleSubmit} className="admin-notice-edit-form">
          ...
          <div className="admin-notice-edit-form-group">
            ...
            <div className="notice-id-search-bar">
              ...
              />
              <button type="button" onClick={handleSearch} className="notice-id-search-btn">
                조회
              </button>
            </div>
          </div>
          <div className="admin-notice-edit-form-group">
            <label>작성자</label>
            <input type="text" value={writerName} disabled className="readonly-field" />
          </div>
          <div className="admin-notice-edit-form-group">
            <label>작성일</label>
            <input type="text" value={createdDate} disabled className="readonly-field" />
          </div>
          <div className="admin-notice-edit-form-group">
            <label>수정일</label>
            <input type="text" value={updatedDate} disabled className="readonly-field" />
          </div>
          <div className="admin-notice-edit-form-group">
            <label>상태</label>
          ...
          <div className="admin-notice-edit-form-buttons">
            <button type="submit" className="admin-notice-edit-save-button">수정</button>
            <button type="button" onClick={handleCancel} className="admin-notice-edit-cancel-button">
              취소
            </button>
          </div>
        </form>
